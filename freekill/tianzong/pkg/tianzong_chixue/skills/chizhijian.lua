...
Fk:addQmlMark{
  name = "chizhijian_swap",
  how_to_show = function(name, value, p)
    if not p then return "" end
    local a = p:getMark("chizhijian_swap_a-turn")
    local b = p:getMark("chizhijian_swap_b-turn")
    if type(a) ~= "number" or type(b) ~= "number" or a == 0 or b == 0 then return "" end
    return zhijian_num_text(a) .. "-" .. zhijian_num_text(b)
  end,
}
...
local chizhijian = fk.CreateSkill{
  name = "chizhijian",
}
local OPT_MARK = "chizhijian1_options-round"
...
chizhijian.addZhijianOnce = function(room, player, expected_delta)
  local opts = player:getTableMark(OPT_MARK)
  ...
  if true then
    ...
    local a2, b2 = zhijian_pick_manual_two(room, player, opts, "chizhijian")
    a, b = a2, b2
...
chizhijian:addAcquireEffect(function(self, player, is_start)
  zhijian_init_opts(player, OPT_MARK, 1)
end)

chizhijian:addEffect(fk.EventPhaseStart, {
  anim_type = "control",
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(chizhijian.name) and player.phase == Player.Start and #player:getTableMark(OPT_MARK) >= 2
  end,
...
chizhijian:addEffect("targetmod", {
...
chizhijian:addEffect(fk.RoundStart, {
  can_refresh = function(self, event, target, player, data)
    return player:hasSkill(chizhijian.name, true)
  end,
