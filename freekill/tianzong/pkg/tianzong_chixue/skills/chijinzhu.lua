local chijinzhu = fk.CreateSkill{
  name = "chijinzhu",
}
...
local function choose_targets_for_virtual(room, player, card_name)
  ...
  local tos = room:askToChoosePlayers(player, {
    ...
    targets = valid,
    skill_name = chijinzhu.name,
    prompt = "#chi_jinzhu_target:::" .. card:toLogString(),
...
chijinzhu:addEffect("active", {
  ...
  interaction = function(self, player)
    local remain = player:getMark("chi_jinzhu_remain")
  ...
  on_use = function(self, room, effect)
    ...
    room:throwCard(effect.cards, chijinzhu.name, player, player)
    if player.dead then return end

    while remain > 0 and not player.dead do
      room:setPlayerMark(player, "chi_jinzhu_remain", remain)
      ...
      local choice = room:askToChoice(player, {
        choices = names,
        skill_name = chijinzhu.name,
        prompt = "#chi_jinzhu_choose:::" .. remain,
      })
      ...
      room:useVirtualCard(choice, nil, player, tos, chijinzhu.name, true)
      remain = remain - len
    end

    room:setPlayerMark(player, "chi_jinzhu_remain", 0)
  end,
})
