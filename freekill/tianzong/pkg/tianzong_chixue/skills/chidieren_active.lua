local chidieren_active = fk.CreateSkill{
  name = "chidieren&",
}
...
local function choose_targets_for_virtual(room, player, card_name)
  ...
  local valid = table.filter(room.alive_players, function(p)
    return not player:isProhibited(p, card) and card.skill:modTargetFilter(player, p, {}, card, { bypass_times = true })
  end)
  ...
  local tos = room:askToChoosePlayers(player, {
    ...
    targets = valid,
    skill_name = "chidieren",
    prompt = "#chi_dieren_target:::" .. card:toLogString(),
...
chidieren_active:addEffect("active", {
  anim_type = "offensive",
  prompt = "#chidieren_active",
  card_num = 1,
  target_num = 0,
  ...
  on_use = function(self, room, effect)
    ...
    room:throwCard(effect.cards, "chidieren", player, player)
    ...
    while remain > 0 and not player.dead do
      ...
      local choice = room:askToChoice(player, {
        choices = names,
        skill_name = "chidieren",
        prompt = "#chi_dieren_choose:::" .. remain,
      })
      ...
      room:useVirtualCard(choice, nil, player, tos, "chidieren", true)
      remain = remain - len
    end
  end,
})

return chidieren_active
