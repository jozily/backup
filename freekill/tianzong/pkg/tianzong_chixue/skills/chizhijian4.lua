...
local chizhijian4 = fk.CreateSkill{
  name = "chizhijian4",
}
local OPT_MARK = "chizhijian4_options-turn"
...
local function do_once(room, player)
  ...
  local a, b = zhijian_pick_manual_two(room, player, opts, "chizhijian4")
...
chizhijian4.addZhijianOnce = function(room, player, expected_delta)
  local opts = player:getTableMark(OPT_MARK)
  ...
  if false then
    ...
    local a2, b2 = zhijian_pick_manual_two(room, player, opts, "chizhijian4")
    a, b = a2, b2
...
chizhijian4:addAcquireEffect(function(self, player, is_start)
  zhijian_init_opts(player, OPT_MARK, 4)
end)

chizhijian4:addEffect(fk.EventPhaseStart, {
  anim_type = "control",
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(chizhijian4.name) and player.phase == Player.Start and #player:getTableMark(OPT_MARK) >= 2
  end,
...
chizhijian4:addEffect(fk.Damaged, {
  anim_type = "masochism",
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(chizhijian4.name) and #player:getTableMark("chizhijian4_options-turn") >= 2
  end,
  on_use = function(self, event, target, player, data)
    do_once(player.room, player)
  end,
...
chizhijian4:addEffect("targetmod", {
...
chizhijian4:addEffect(fk.TurnStart, {
  can_refresh = function(self, event, target, player, data)
    return target == player and player:hasSkill(chizhijian4.name, true)
  end,
