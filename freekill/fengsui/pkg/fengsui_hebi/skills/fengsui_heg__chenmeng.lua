...
local function doOption(room, player, option)
  if option == "fengsui_heg__chenmeng_discard" then
    local cards = room:askToDiscard(player, {
      min_num = 2,
      max_num = #player:getCardIds("he"),
      include_equip = true,
      skill_name = chenmeng.name,
      prompt = "#fengsui_heg__chenmeng-discard",
      cancelable = false,
    })
  ...
  elseif canExchange(player) then
    ...
    local mark = room:askToChoice(player, {
      choices = names,
      skill_name = chenmeng.name,
      prompt = "#fengsui_heg__chenmeng-mark",
    })
    local id = room:askToChooseCard(player, {
      target = player,
      flag = "he",
      skill_name = chenmeng.name,
      prompt = "#fengsui_heg__chenmeng-exchange",
    })
    if id then
...
chenmeng:addEffect("active", {
  ...
  on_use = function(self, room, effect)
    ...
    local choices = {}
    if canDiscard(player) then table.insert(choices, "fengsui_heg__chenmeng_discard") end
    if canExchange(player) then table.insert(choices, "fengsui_heg__chenmeng_exchange") end
    local choice = room:askToChoice(player, {choices = choices, skill_name = chenmeng.name,
      prompt = "#fengsui_heg__chenmeng-choice"})
    doOption(room, player, choice)
    local other = choice == "fengsui_heg__chenmeng_discard" and
      "fengsui_heg__chenmeng_exchange" or "fengsui_heg__chenmeng_discard"
    room:setPlayerMark(player, "fengsui_heg__chenmeng_pending-turn", other)
  end,
})
...
chenmeng:addEffect(fk.TurnEnd, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:getMark("fengsui_heg__chenmeng_pending-turn") ~= 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local option = player:getMark("fengsui_heg__chenmeng_pending-turn")
    room:setPlayerMark(player, "fengsui_heg__chenmeng_pending-turn", 0)
