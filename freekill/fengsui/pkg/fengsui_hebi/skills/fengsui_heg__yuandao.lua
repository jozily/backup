...
yuandao:addEffect(fk.CardUseFinished, {
  ...
  on_refresh = function(self, event, target, player, data)
    player.room:setPlayerMark(target, "fengsui_heg__yuandao_active-turn", 1)
  end,
})
...
yuandao:addEffect(fk.PreCardUse, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:getMark("fengsui_heg__yuandao_active-turn") > 0
  end,
  on_use = function(self, event, target, player, data)
    player.room:setPlayerMark(player, "fengsui_heg__yuandao_active-turn", 0)
    data.extraUse = true
  end,
})
