...
local qingsong = fk.CreateSkill{
  name = "fengsui_heg__qingsongRemake",
}
...
qingsong:addEffect(fk.TargetConfirmed, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    local cards = U.chooseCardsFromList(player.room, player, candidates, 1, 1,
      "#fengsui_heg__qingsongRemake-give", qingsong.name, true)
    ...
    local tos = player.room:askToChoosePlayers(player, {
      ...
      prompt = "#fengsui_heg__qingsongRemake-to",
      cancelable = true,
    })
    if #tos > 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    if skill_place ~= nil then player:hideGeneral(not skill_place) end
    room:moveCardTo(cost.card, Card.PlayerHand, cost.to, fk.ReasonGive,
      qingsong.name, nil, true, player)
    if not H.compareKingdomWith(player, cost.to) then
      local card_name = Fk:getCardById(cost.card).trueName
      room:addTableMark(player, "fengsui_heg__qingsongRemake_use-turn", card_name)
      room:addTableMarkIfNeed(player, "@fengsui_heg__qingsongRemake-turn", card_name)
    end
  end,
})
...
qingsong:addEffect(fk.TurnEnd, {
  global = true,
  can_trigger = function(self, event, target, player, data)
    return #player:getTableMark("fengsui_heg__qingsongRemake_use-turn") > 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local cards = player:getTableMark("fengsui_heg__qingsongRemake_use-turn")
    room:setPlayerMark(player, "fengsui_heg__qingsongRemake_use-turn", 0)
    for _, name in ipairs(cards) do
      if player:isAlive() then
        room:askToUseVirtualCard(player, {
          name = name,
          skill_name = qingsong.name,
          prompt = "#fengsui_heg__qingsongRemake-use",
          cancelable = true,
