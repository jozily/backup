...
local guishang = fk.CreateSkill{
  name = "fengsui_heg__guishang",
}

guishang:addEffect("active", {
  prompt = "#fengsui_heg__guishang",
  ...
  target_filter = function(self, player, to_select, selected)
    return #selected == 0 and
      not table.contains(player:getTableMark("fengsui_heg__guishang_targets-turn"), to_select.id)
  end,
  ...
  on_use = function(self, room, effect)
    ...
    room:addTableMark(player, "fengsui_heg__guishang_targets-turn", target.id)
    ...
    local choice = "fengsui_heg__guishang_gain"
    if #discardable > 0 then
      choice = room:askToChoice(target, {
        choices = {"fengsui_heg__guishang_gain", "fengsui_heg__guishang_discard_use"},
        prompt = "#fengsui_heg__guishang-choice",
        skill_name = guishang.name,
      })
    end
    if choice == "fengsui_heg__guishang_gain" then
    ...
    local discarded = U.chooseCardsFromList(room, target, discardable, 1, #discardable,
      "#fengsui_heg__guishang-discard", guishang.name, false)
  ...
  on_refresh = function(self, event, target, player, data)
    ...
    for _, move in ipairs(data) do
      if move.to and move.toArea == Card.PlayerHand and move.from ~= move.to then
        room:addPlayerMark(move.to, "fengsui_heg__guishang_obtained-turn", #move.moveInfo)
      end
    end
  end,
})
...
guishang:addEffect(fk.EventPhaseEnd, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    table.sort(sorted, function(a, b)
      return a:getMark("fengsui_heg__guishang_obtained-turn") >
        b:getMark("fengsui_heg__guishang_obtained-turn")
    end)
    if #sorted < 2 then return end
    local most, least = sorted[1], sorted[#sorted]
    if most:getMark("fengsui_heg__guishang_obtained-turn") ==
      sorted[2]:getMark("fengsui_heg__guishang_obtained-turn") then return end
    if least:getMark("fengsui_heg__guishang_obtained-turn") ==
      sorted[#sorted - 1]:getMark("fengsui_heg__guishang_obtained-turn") then return end
