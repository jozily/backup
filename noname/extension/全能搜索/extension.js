...
export default () => {
	return {
        ...
        async content(config, pack) {
            ...
            // const { Pagination } = await import('./SimplePagination.js');
            ...
            class Searcher {
                    ...
                    for (const data of name) {
                        ...
                        const getImageUrl = ($0) => {
                            ...
                            return $0.style.backgroundImage.split(',').map(v => {
                                ...
                                return typeof v == "string" && !v.includes(`image/character/default_silhouette_male.jpg`) && !v.includes(`image/character/default_silhouette_female.jpg`);
                            })[0];
                        };
                        ...
                        for (const skill of skills) {
                            ...
                            try {
                                ...
                                game.alert('对不起，检测您的浏览器太low了，建议您升级浏览器后，解锁更多新功能');
                            ...
                            if (lib.translate[skill]) {
                                /** 技能台词html字符串 */
                                let skillText = game.全能搜索_createSkillText(skill, charName);
                        ...
                        for (const derivation of skillDerivation) {
                            /** 技能台词html字符串 */
                            let skillText = game.全能搜索_createSkillText(derivation, charName);
                            ...
                            skillDerivationstr +=
                                html`<li>
                                    ...
                                    ${typeof lib.translate[derivation + '_append'] == 'string' ? (lib.translate[derivation + '_append'] + '</br>') : ''}
                                    ${skillText}
                                    <span class="bluetext">技能代码&nbsp</span>
                                    <a onclick='window.qnssShowCode.call(this)'>点击查看技能代码</a>
                                </li>
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            /** 阵亡台词html字符串 */
                            let dieText = game.全能搜索_createDieText(charName);
                            ...
                            d.insertAdjacentHTML('afterbegin', html`
                                ...
                                <span class="bluetext">${String(character[2]).includes("/") ? "体力值/体力上限/护甲" : "体力上限"}&nbsp</span><span>${character[2]}</span>
                                </br>
                        ...
                        */
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            d.insertAdjacentHTML('afterbegin', html`
                                ...
                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextSibling)">卡牌效果&nbsp</span>${nature ? lib.card.sha.cardPrompt({ name: 'sha', nature }) : lib.translate[cardName + '_info']}
                                </br>
                                
                                <span class="bluetext" ondblclick="game.全能搜索_copy(this.nextElementSibling.nextElementSibling)">所在卡牌包&nbsp</span>${packTranslate || '无'}
                                <font color=6df95b>[ </font>
                ...
                findSkill(result) {
                    ...
                    for (let i = 0; i < skills.length; i++) {
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            /** 技能台词html字符串 */
                            let skillText = game.全能搜索_createSkillText(skillName);
                            ...
                            d.insertAdjacentHTML('afterbegin', html`
                                <li>
                                    ...
                                    ${typeof lib.translate[skill.skillName + '_append'] == 'string' ? (lib.translate[skill.skillName + '_append'] + '</br>') : ''}
                                    ${skillText}
                ...
                findCode(result) {
                    ...
                    if (!description && typeof game.getFileList == 'function' && typeof window.require !== 'undefined') {
                        try {
                            import('./generateDocumentation.js').then(({ init }) => {
                                init().then(result => {
                                    description = result;
                                });
                            })
                    ...
                    if (exceptResult) {
                        ...
                        function stringify(obj, level) {
                            if (get.objtype(obj) == 'object' || typeof obj == 'function' || (Array.isArray(obj) && obj.includes(Infinity))) {
                                return get.stringify(obj, level);
                            }
                            else {
                                let str;
                        ...
                        if (Array.isArray(description) && exceptLast[0] in qnssVariable) {
                            // 对应变量
                            let localVariable = qnssVariable[exceptLast[0]];
                            // 对应注释
                            let localDescription = description.find(v => {
                                if (v.name != 'default') {
                                    if (typeof localVariable != 'function') {
                                        return v.name == localVariable?.constructor?.name;
                                    }
                                    else {
                                        return v.name == localVariable.name;
                                    }
                                }
                                else {
                                    return v.instanceMembers.every(({ name }) => Object.hasOwn(localVariable, name));
                                }
                            });
                            try {
                                if (localDescription) {
                                    for (const key of result.split('.').slice(1)) {
                                        const previousVariable = localVariable;
                                        localVariable = localVariable[key];
                                        if (localVariable === undefined) break;
                                        if (localDescription.instanceMembers) {
                                            localDescription = localDescription.instanceMembers.find(v => {
                                                return v.name == key;
                                            });
                                        }
                                        else {
                                            const previousDescription = description.find(v => {
                                                if (v.name != 'default') {
                                                    if (typeof previousVariable != 'function') {
                                                        return v.name == previousVariable?.constructor?.name;
                                                    }
                                                    else {
                                                        return v.name == previousVariable.name;
                                                    }
                                                }
                                                else {
                                                    return v.instanceMembers.every(({ name }) => previousVariable[name]);
                                                }
                                            });
                                            if (previousDescription.instanceMembers) {
                                                localDescription = previousDescription.instanceMembers.find(v => {
                                                    return v.name == key;
                                                });
                                            }
                                        }
                                        if (localDescription === undefined) break;
                                    }
                                }
                            } catch(e) {
                                console.error(e);
                            }
                            // console.log(obj === localVariable, {
                            //     localVariable,
                            //     localDescription
                            // });
                            if (obj === localVariable && localDescription) {
                                descriptionStr += html`<span>
                                ${typeof obj == 'function' ? '函数' : '变量'}描述: ${localDescription.documentation || '无'}</br>
                                ${typeof obj == 'function' ? '函数返回值' : '变量'}类型: ${localDescription.type}</br>`;

                                if (typeof obj == 'function' && Array.isArray(localDescription.parameters) && localDescription.parameters.length > 0) {
                                    const borderStyle = `style="border: 1px solid #000000"`;
                                    descriptionStr += `函数参数: </br>`;
                                    descriptionStr += html`<table style="border-collapse: collapse">
                                        <thead>
                                            <tr>
                                            <th scope="col" ${borderStyle}>参数名称</th>
                                            <th scope="col" ${borderStyle}>参数类型</th>
                                            <th scope="col" ${borderStyle}>参数描述</th>
                                            </tr>
                                        </thead>
                                        <tbody>`;

                                    for (const { name, type, documentation } of localDescription.parameters) {
                                        descriptionStr += html`
                                        <tr>
                                            <td ${borderStyle}>${name}</td>
                                            <td ${borderStyle}>${type}</td>
                                            <td ${borderStyle}>${documentation || '无'}</td>
                                        </tr>`;
                                    }
                                    
                                    descriptionStr += html`</tbody></table>`;
                                }
                        ...
                        this.fragmentDataList[this.fragmentList.length - 1].push((lastFragment) => {
                            ...
                            d.insertAdjacentHTML('afterbegin', `
                                <li>
                                    ...
                                    ${descriptionStr}
                                    <pre class="hljs language-javascript" style="user-select:text;-webkit-user-select:text;">${'// ' + result + '的源代码: </br>'}${game.全能搜索_highlight(stringify(obj))}</pre>
                                </li>
                            `);
        ...
        config: {
            version: {
                nopointer: true,
                clear: true,
                name: "扩展版本: v3.6<br>更新日期: 2025-02-19",
            },
            css: {
                clear: true,
                'name': '更换代码高亮样式',
